...
(function (Aufgabe8) {
    ...
    function writeHTML() {
        ...
        for (let i = 0; i < Aufgabe8.tree.length; i++) {
            childNodeHTML += "<option value='" + i + Aufgabe8.tree[i].name + "'>" + Aufgabe8.tree[i].name + "</option>";
        }
        childNodeHTML += "</select>";
        childNodeHTML += "<br>";
        childNodeHTML += "<h3>Glasskugeln</h3>";
        for (let i = 0; i < Aufgabe8.christmasBall.length; i++) {
            childNodeHTML += Aufgabe8.christmasBall[i].name;
            childNodeHTML += " <input type='number' id='numberBalls" + i + "' name='Glasskugeln' step='1' min='0' max='30' value='0' title='" + Aufgabe8.christmasBall[i].name + "' price='" + Aufgabe8.christmasBall[i].price + "'/>";
            childNodeHTML += "<br>";
            continue;
        }
        //Kerzen
        childNodeHTML += "<h3>Kerzen</h3>";
        for (let i = 0; i < Aufgabe8.candle.length; i++) {
            childNodeHTML += Aufgabe8.candle[i].name;
            childNodeHTML += " <input type='number' id='numberCandles" + i + "' name='Kerzen' step='1' min='0' max='30' value='0' title='" + Aufgabe8.candle[i].name + "' price='" + Aufgabe8.candle[i].price + "' />";
            childNodeHTML += "<br>";
            continue;
        }
        childNodeHTML += "<h3>Lametta</h3>";
        for (let i = 0; i < Aufgabe8.lametta.length; i++) {
            childNodeHTML += Aufgabe8.lametta[i].name;
            childNodeHTML += " <input type='number' id='numberLametta" + i + "' name='Lametta' step='1' min='0' max='30' value='0' title='" + Aufgabe8.lametta[i].name + "' price=" + Aufgabe8.lametta[i].price + " />";
            childNodeHTML += "<br>";
            continue;
        }
        childNodeHTML += "<h3>Halterung</h3>";
        childNodeHTML += "<select name='Halterung' id='holder'>";
        for (let i = 0; i < Aufgabe8.holder.length; i++) {
            childNodeHTML += "<option value='" + i + Aufgabe8.holder[i].name + "'>" + Aufgabe8.holder[i].name + "</option>";
        }
        childNodeHTML += "</select>";
        childNodeHTML += "<br>";
        childNodeHTML += "<h3>Lieferant</h3>";
        childNodeHTML += "<select name='Lieferant' id='shipment'>";
        for (let i = 0; i < Aufgabe8.shipment.length; i++) {
            childNodeHTML += "<option value='" + i + Aufgabe8.shipment[i].name + "'>" + Aufgabe8.shipment[i].name + "</option>";
        }
        childNodeHTML += "</select>";
        childNodeHTML += "<br>";
    ...
    function handleChange(_event) {
        ...
        if (target.id == "tree") {
            ...
            let childNodeHTML;
            treePrice = Aufgabe8.tree[priceIndex].price;
            childNodeHTML = "";
        ...
        if (target.id == "holder") {
            ...
            let priceIndex = parseInt(value.substr(0, 1));
            let childNodeHTML;
            holderPrice = Aufgabe8.holder[priceIndex].price;
            childNodeHTML = "";
            childNodeHTML += "<a name=" + target.name + " value=" + value.substr(1) + ">";
            childNodeHTML += " " + value.substr(1);
            childNodeHTML += "</a>";
        ...
        if (target.id == "shipment") {
            ...
            let priceIndex = parseInt(value.substr(0, 1));
            let childNodeHTML;
            shipmentPrice = Aufgabe8.shipment[priceIndex].price;
            childNodeHTML = "";
            childNodeHTML += "<a name=" + target.name + " value=" + value.substr(1) + ">";
            childNodeHTML += " " + value.substr(1);
            childNodeHTML += "</a>";
